...
public class ConfigLoaderFromResource implements ConfigLoader {
	...
	protected void load(Properties props, List<String> modeList){
		...
		if(props == null || modeList == null || modeList.size() <= 0){
			return;
		}
		
		if (BuildConfig.DEBUG) {
			Log.d(TAG, "load config from resource stream");
		}
		
		String pathPrefix = "/" + App.Instance().getPackageName().replace('.', '/');
		
		for(String mode : modeList){
			String confPath = pathPrefix + "/config/" + mode + ".properties";
			if (BuildConfig.DEBUG) {
				Log.d(TAG, "confPath=" + confPath);
			}
			InputStream input = null;
			try {
				input = App.class.getResourceAsStream(confPath);
